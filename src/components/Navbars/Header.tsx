          ...
          <nav className="flex items-center gap-8">
            {headerNavLinks.slice(0, 3).map((link) => (
              <Link
                key={link.id}
                href={link.href}
                className={`text-sm font-medium uppercase tracking-[0.15em] transition relative group ${
                  pathname === link.href ?
                    "text-black"
                ...
                {link.text}
                <span
                  className={`h-[2px] inline-block bg-black absolute left-0 -bottom-1 transition-all ease duration-300 ${
          ...
          <div className="flex items-center gap-8">
            ...
            <div className="flex items-center gap-5 ml-4 border-l border-gray-200 pl-6">
              ...
              <Menu as="div" className="relative inline-block text-left">
                {({ open }) => (
                  <>
                    <Menu.Button className="flex items-center cursor-pointer group outline-none focus:ring-0 text-gray-600 hover:text-black transition">
              ...
              <div
                ...
                onClick={onOpenCart}>
                <FiShoppingBag className="text-lg text-gray-600 group-hover:text-black transition" />
                {totalItems > 0 && (
                  <span className="absolute -top-1.5 -right-1.5 size-4 bg-black text-white text-[9px] font-black flex items-center justify-center rounded-full">
                    {totalItems}
                  </span>
                )}
              </div>
        ...
        <div className="slg:hidden flex flex-col w-full px-3 sm:px-4 py-3 sm:py-4 gap-3 bg-black">
          ...
          <div className="relative h-10">
            <input
              ...
              className="w-full h-full text-sm bg-gray-100 rounded-lg px-4 border-none outline-none focus:ring-2 focus:ring-primary-100 touch-manipulation"
              value={searchValue}
