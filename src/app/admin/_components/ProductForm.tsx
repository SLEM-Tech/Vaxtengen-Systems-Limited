...
export default function ProductForm({
	...
	const [form, setForm] = useState<ProductFormData>({
		...
		attributes:
			(initial?.attributes as any[])?.map((a: any) => ({
				name: a.name,
				options: Array.isArray(a.options) ? a.options.join(", ") : "",
			})) ?? [],
		categories:
			(initial?.categories as any[])?.map((c: any) => c.id ?? c) ?? [],
	});
	...
	function updateAttribute(i: number, field: "name" | "options", val: string) {
		set(
			"attributes",
			form.attributes.map((a, idx) => (idx === i ? { ...a, [field]: val } : a)),
		);
	}
					...
					<div className='bg-white rounded-xl border border-gray-100 p-5 space-y-4'>
						...
						<div>
							...
							<textarea
								...
								rows={2}
								className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-100/30 resize-none'
							...
							<div key={i} className='grid grid-cols-5 gap-3 items-start'>
								...
								<div className='col-span-2'>
									<input
										value={attr.options}
										onChange={(e) =>
											updateAttribute(i, "options", e.target.value)
										}
							...
							{categories.map((cat) => (
								...
								>
									<input
										...
										className='rounded border-gray-300 text-primary-100'
									/>
