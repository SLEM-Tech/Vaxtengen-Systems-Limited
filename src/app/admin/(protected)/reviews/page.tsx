...
export default function ReviewsPage() {
	...
	const { data: productsData } = useQuery({
		...
		queryFn: () =>
			fetch("/api/admin/products?per_page=200&status=publish").then((r) =>
				r.json(),
			),
	});
				...
				<div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
					...
					<div className='space-y-2'>
						...
						<input
							...
							className='w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-100/30'
						/>
					...
					<div className='space-y-4'>
						<div className='space-y-2'>
							...
							<p className='text-xs text-gray-400'>
								Between 1 and 200 reviews per product.
							</p>
						</div>

						{selectedProducts.length > 0 && (
							<div className='bg-blue-50 border border-blue-100 rounded-lg p-3 text-xs text-blue-700'>
								Will generate <strong>{count * selectedProducts.length}</strong>{" "}
								total reviews across <strong>{selectedProducts.length}</strong>{" "}
								product(s).
							</div>
						)}
						...
						<button
							...
							disabled={
								selectedProducts.length === 0 || generateMutation.isPending
							}
			...
			<div className='space-y-4'>
				<div className='flex flex-col sm:flex-row sm:items-center gap-3'>
					...
					<input
						...
						className='px-3 py-2 border border-gray-200 rounded-lg text-sm w-60 focus:outline-none focus:ring-2 focus:ring-primary-100/30'
					/>
					<select
						...
						className='px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-100/30'
					>
				...
				<div className='flex items-center justify-between'>
					<p className='text-sm text-gray-500'>
						{reviewsData?.total ?? 0} reviews total
					</p>
					<AdminPagination
						page={page}
