...
export default function BannersPage() {
	...
	const deleteMutation = useMutation({
		mutationFn: (id: number) =>
			fetch(`/api/admin/banners/${id}`, { method: "DELETE" }).then((r) =>
				r.json(),
			),
		onSuccess: () => {
			qc.invalidateQueries({ queryKey: ["admin-banners"] });
					...
					<form onSubmit={handleSubmit} className='space-y-4'>
						...
						<label className='flex items-center gap-2 cursor-pointer'>
							<input
								...
								checked={form.show}
								onChange={(e) =>
									setForm((f) => ({ ...f, show: e.target.checked }))
								}
					...
					{banners.map((b) => (
						...
						>
							<div className='relative aspect-[16/6]'>
								...
								/>
								<div
									className={`absolute top-2 right-2 px-2 py-0.5 rounded-full text-xs font-medium ${b.show ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
								>
							...
							<div className='p-3'>
								...
								<div className='flex gap-2 mt-3'>
									<button
										onClick={() =>
											toggleMutation.mutate({ id: b.id, show: !b.show })
										}
									...
									<button
										onClick={() => startEdit(b)}
										className='flex-1 py-1.5 text-xs bg-primary-100 text-white rounded hover:bg-[#003a73]'
									>
										Edit
									</button>
									<button
										onClick={() => setDeleteId(b.id)}
										className='flex-1 py-1.5 text-xs bg-red-50 text-red-600 rounded hover:bg-red-100 border border-red-100'
									>
										Delete
									</button>
								</div>
